...
npm run test:server # node:test 서버 330건 (검증·서비스·저장소·인증·감사)
...
> **코드 변경 후 `npm run verify` 필수** (lint 0 error + 단위 594건 + 서버 330건 + 빌드 + E2E 122건).
...
```bash
npm run verify           # lint + 단위 594건 + 서버 330건 + 빌드 + E2E 122건
