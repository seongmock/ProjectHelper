...
npm run test:unit    # Vitest — 도메인 순수함수 + XSS 회귀 (173건)
npm run test:server  # node:test — 검증·서비스·저장소 내구성·감사 로그 (97건)
npm run test:e2e     # Playwright E2E 37건 (API·dev 서버 자동 기동)
...
**변경 후에는 `npm run verify`** — 합격 기준은 lint 0 error · unit 173/173 · server 97/97 ·
빌드 성공 · **E2E 37/37 (skip 0)**.
