...
# 4) E2E 34건 — playwright.config.js 가 API·dev 서버를 자동 기동한다
npx playwright test
...
**합격 기준**: lint 0 error · unit 150/150 · server 97/97 · 빌드 성공 · **E2E 35/35(skip 0)**.
...
| P3-5 | 인스펙터 패널 + 정보구조 재설계 | 🔶 | 인스펙터 패널 **v1 완료**(2026-08-07, 아래 UX 표 참조). 남은 것은 ① 팝오버 3종을 패널로 흡수하고 폐기 ② 정보구조 재설계(§5.4-11, 별도 항목) |
...
| 실사 §5.4-12 인스펙터 패널 (= P3-5) | 🔶 2026-08-07 | **v1 완료** — `features/tasks/InspectorPanel.jsx` + 파생 계산 순수함수 `summarizeTask()`. 툴바 우측 상시 버튼으로 토글(`showInspector` 설정에 저장). **팝오버 3종은 아직 제거하지 않았다** — 기간별 라벨·색, 의존성 연결 시작이 거기 남아 있다. 그 흡수가 v2 |
...
> `.timeline-popover` 존재 여부로 판단). 새 오버레이를 만들면 그 클래스를 쓰거나
