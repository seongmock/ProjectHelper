...
| 저장 엔진 이중화 | 운영은 SQLite(2026-08-20). 검증은 두 겹 — 저장소 계층은 미러 테스트(`sqliteStore.test.js`)가, HTTP 경로는 E2E 가 두 엔진으로 돈다(`npm run test:e2e:sqlite`, CI `matrix.store`). 남은 위험은 엔진 설정 누락이고 `verify-deploy.sh` [15] 가 본다 |
