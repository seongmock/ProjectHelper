...
- 사용자는 **좌측 프로젝트 레일**에서 클릭 한 번으로 전환해 확인한다. 레일은 리비전 폴링과 함께 목록을 다시 읽으므로 AI가 만든 프로젝트가 새로고침 없이 나타난다(2026-08-19 이전의 헤더 드롭다운은 삭제됐다).
...
**포트 3000이 비어 있다는 보장은 없다.** 개발 호스트에 따라 무관한 서비스가 점유 중일 수
있고(이 저장소를 개발한 호스트가 그렇다 — uvicorn), 그러면 `.mcp.json`의 기본값
`PH_API_BASE=http://localhost:3000/api`는 **엉뚱한 서버에 물어보고 404를 돌려준다**. 붙기
전에 `curl -s localhost:3000/api/health`로 우리 서버인지부터 확인하고, 다르면 둘을 함께
옮긴다: `PORT=3100 npm run dev:api` + `PH_API_BASE=http://localhost:3100/api`. MCP가 조용히
빈 손으로 돌아오는 대부분의 원인이 이것이다.

**앱을 쓰는 사람은 이 문서를 읽지 않는다** — 그래서 같은 내용의 진입점이 화면에도 있다:
헤더의 `AI 가이드`(Bot 아이콘) → *"AI 가 직접 편집하게 하기 (API)"*. 거기서 **지금 보고 있는
프로젝트의 id 와 이 배포의 API 주소가 박힌** 문장을 복사해 AI 세션에 그대로 붙여넣을 수 있고,
`/api/guide`·`/api/openapi.yaml` 링크가 함께 있다. 그 화면은 사용법을 **다시 쓰지 않는다**
(사본은 드리프트한다) — 값과 링크만 만들어 주고 설명은 `/api/guide`에 맡긴다.
